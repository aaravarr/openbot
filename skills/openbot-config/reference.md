...
The fusion gateway edge allows a **10 MiB** request body, but its upstream rejects real-world payloads below that (a ~9.2 MB body came back **413 "Request Entity Too Large"** on 2026-09-04), so the binding limit lives under the edge. Injected base64 inflates images by ~33%. Before dispatch, `payload/image-read.cjs` compresses every data-URI image (whatever its source: harness bytes, disk read, or user attachment) and enforces an **8 MiB** budget on `body.messages` (measured as UTF-8 bytes):
...
- **Budget**: if the serialized messages still exceed 8 MiB, images are down-stepped **in document order — oldest history image first, so the newest (current-turn) image keeps its quality last** (quality q85→q70→q50, then size 1568→1024→768; each image steps through each rung at most once). If it still does not fit, the **oldest image first** in document order is replaced with the text `[image omitted: budget]` — the newest/current-turn image is dropped last — preserving message order; the request never 413s. Each step logs one stderr line with before/after bytes. Text-only or under-budget requests never spawn the converter probe.
