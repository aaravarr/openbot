...
test("a paused bot does not block other bots, and a corrupt state file lets requests through", async () => {
  const botA = "123e4567-e89b-12d3-a456-426614174000";
  const botB = "223e4567-e89b-12d3-a456-426614174001";
  const fromBot = (botId: string) => ({
    model: "m",
    messages: [
      { role: "system", content: "/home/box/agent-data/agents/" + botId + "/profile.json" },
  ...
  const dir = mkdtempSync(path.join(os.tmpdir(), "openbot-bot-pause-miss-"));
  const old = {
  ...
  writeFileSync(process.env.OPENBOT_PLAN, JSON.stringify({
    kind: "custom",
    catalog: {
      providers: [{ id: "p", name: "P", origin: "http://127.0.0.1:" + String(upstream.port) + "/v1", maxTokensDefault: 100, mapFile: "provider-maps.cjs" }],
      models: [{ id: "p:m", providerId: "p", slug: "m", parameters: [] }],
      bindings: [{ conversation: { kind: "wildcard" }, modelId: "p:m" }],
    },
  }));
  writeFileSync(process.env.OPENBOT_SECRETS, JSON.stringify({ providers: { p: "sk-test" } }));
  writeFileSync(process.env.OPENBOT_LOGS, JSON.stringify({ loggingEnabled: false }));
  const server = await listen((req, res) => { void hop.handleHopRequest(req, res); });
  try {
    writeFileSync(path.join(dir, "openbot-pause-bots.json"), JSON.stringify({ pausedBotIds: [botA] }));
    const otherBot = await post(server.port, fromBot(botB));
    assert.equal(otherBot.status, 200);
    assert.equal(upstreamHits, 1);

    writeFileSync(path.join(dir, "openbot-pause-bots.json"), "{broken");
    const failOpen = await post(server.port, fromBot(botA));
