...
Custom providers may set `apiType` to `chat-completions` (the backward-compatible default), `responses`, or `anthropic`. Grok Bot always speaks Chat Completions to OpenBot. Responses providers use `/v1/responses`, Responses `input`/function-call items, and are converted back from text, reasoning, tool-call, usage, and completion events. Anthropic providers use `/v1/messages`, `anthropic-version: 2023-06-01`, and `x-api-key`; system messages, `tool_use`/`tool_result`, thinking blocks, stop reasons, usage, and SSE lifecycle events are mapped back to Chat Completions. `max_tokens` is governed against the model row before conversion; Responses receives the clamped `max_output_tokens`, Anthropic receives required `max_tokens`. Missing `apiType` in an existing plan means `chat-completions`. Long tool-call IDs are clamped by UTF-8 byte length (maximum 64 bytes) consistently in the TypeScript and payload converters. Responses `content_filter` takes precedence over generic `incomplete`/length mapping. SSE conversion accepts consecutive `data:` lines even when an upstream omits blank event separators.
