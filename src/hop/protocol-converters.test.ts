...
test("Anthropic request and JSON response map system, tool blocks, thinking, and cache usage", () => {
  ...
  const result = anthropicToChat({ id: "msg_1", model: "m", content: [{ type: "thinking", thinking: "private" }, { type: "text", text: "answer" }, { type: "tool_use", id: "toolu_1", name: "lookup", input: { q: "x" } }], stop_reason: "tool_use", usage: { input_tokens: 2, cache_read_input_tokens: 3, output_tokens: 4 } }) as any;
  assert.equal(result.choices?.[0]?.finish_reason, "tool_calls");
