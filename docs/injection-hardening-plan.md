...
The strategy is deliberately outside toOpenAIMessages. That function remains a pure structural conversion. SendToUser and SendMessage count as touches; ReactToMessage also counts as a touch under the official ledger, although it does not deliver content. The classifier's touch signal is the **gateway-observed initiated delivery call**; host delivery events are optional and never gate a layer. Detecting and re-sending a failed delivery is out of scope: the official harness owns that retry. It never maps leftover text to SendToUser, never forces finish_reason, and never claims delivery from plain text or a call alone.
...
- sentMessageCount: a known count only when a trusted host event/snapshot reports successful send-message delivery; otherwise unknown. It is optional accounting, not a policy gate: the shipped classifier runs on the gateway-observable fields below.
...
2. The request must be identifiable as a person-opened real-user turn from what the hop can observe. When the host/transport supplies hidden, requestSource, isSubagent, isSilenceAllowed, routine/automation, chatType, and group-state fields, use them. When it does not, fall back to observable request signals (a present bot identity, a direct-chat conversation, and absence of the internal summary/memory lane) and record the residual uncertainty in the decision record. Group turns are always excluded, as are turns positively identified as hidden, subagent, routine/automation, or silence-allowed. Identity fields are never invented, and no flag may be promoted from an unverified source.
3. botId and conversationId must be non-empty from the request as the hop observes it (host-supplied when available, otherwise the observed request identity). An empty or missing bot id, including the 17:45:50 memory call, is unconditionally ineligible. The ledger key is the observed conversation + bot identity, plus a host-supplied epoch when one is available.
4. L2 and L3 use the same latest-real-user debt classifier and shape selector defined in §§7.1 and 9.3. The default touch signal is the gateway-observed initiated delivery call (call-emitted); host delivery events are an optional precision upgrade, not a prerequisite. Modelling a failed send and re-sending after it is explicitly out of scope because the official harness owns delivery retry and its own failure remediation. Missing latest-user boundary state skips the layer rather than inventing one. L1 continues to use the official stream thresholds in §6.1 and does not acquire the L2 terminal lease.
...
1. **Request permission gate (inbound):** the request is a person-opened real-user turn. Use host/transport identity fields when the host supplies them; otherwise use the observable signals described in §5.2 (present bot identity, direct-chat conversation, no internal summary/memory lane) and record the uncertainty. A request that cannot be positively separated from the group, hidden, subagent, routine/automation, or silence-allowed lanes does not pass. A failed gate means no intervention at all.
...
Official semantics treat “attempted but not delivered” as owed, but modelling that is out of scope here: retrying after a failed send belongs to the official harness, which already tracks it and re-drives it. The shipped classifier therefore treats a gateway-observed initiated delivery call as the touch. When a trusted host delivery event happens to be available it is recorded and shown in the decision record as delivery-observed, but no layer blocks on it, and the gateway must never claim successful delivery from a call alone.
...
~~~text
touch =
  call-emitted events for SendToUser | SendMessage | ReactToMessage   # default signal
    (host delivery-observed events may additionally be recorded, but the
     classifier does not require them and does not block on their absence)
...
7. **Delivery-failure modelling is out of scope:** the classifier counts an initiated delivery call as a touch and does not try to detect a send that failed. Rationale: the official harness already tracks attempted-but-undelivered sends and re-drives them (§3.27's wording covers it), so duplicating that here would risk double-sending. Residual risk: a silently failed send followed by a plain-text ending would be classified as touched and not nudged; record delivery as unknown rather than claiming success.
8. **Identity metadata:** prefer the authenticated HostRequestContext/transport envelope defined in §9.1 and wire botId, conversationId, epoch, person/source flags, and chatType/group state from it when present. When the host does not supply it, use the observable request signals in §5.2 and record the residual uncertainty; never promote an unverified flag into the gate. The empty-bot memory record remains the canonical negative example.
