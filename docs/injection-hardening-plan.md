...
- **L2 — after generation:** the key patch for this incident. The upstream response is converted into host-boundary chat-completions events incrementally. Every text, reasoning, and tool-call delta is forwarded immediately. Only the terminal event or events carrying finish_reason are held. At that point, a local decision checks the trusted person-opened permission gate, the delivery-call/tool history, and whether the terminal reason is stop. Ordinary turns release the held terminal immediately. A silent plain-text stop tail may instead start exactly one second upstream run in the same host request, with the exact §3.27 hidden suffix appended to the canonical context; the second response continues the same host stream and supplies its terminal close. Any second-run failure releases the original held terminal immediately. There is no whole-response capture or response-size admission rule; provider continuation is not used.
...
Normal turns — a turn whose delivery is after the last tool execution, or whose terminal event carries tool calls — have zero extra model calls, zero response buffering, and no first-byte delay. The only work is a local, millisecond-scale observation and the immediate release of the tiny terminal event. The same is true for non-person, hidden, group, subagent, routine, and silence-allowed turns: the permission gate skips intervention while the ordinary stream remains unchanged.

Only a turn that would otherwise end in a silent plain-text stop can incur a second model call. Its body has already been streamed to the host, so the extra wait applies to a turn that had no user-visible delivery at the point of the decision; it does not delay the first byte of normal productive turns. The incident-shaped exception is intentional: an opening delivery acknowledgement followed by later tool execution is still a silent final result unless a later delivery call occurred after that execution.
...
At the host boundary, the incremental reverse adapter supplies normalized chat-completions events. The classifier keeps only bounded counters, IDs, event sequence numbers, and the temporal delivery/tool history needed below. It does not retain the response body.
...
2. The terminal event set is complete and every relevant choice ends with finish_reason="stop". A terminal finish_reason="tool_calls" is not eligible. A terminal event that itself carries tool_calls is also not eligible; release it immediately.
3. The response ends in a pure-text silent tail: the terminal tail contains ordinary text/reasoning or no further content, not a terminal tool-call event. Earlier tool execution does not by itself make the turn ineligible.
4. Where the trusted host ledger is available, deliveryOwed is explicitly true: sentMessageCount === 0 and reacted === false. Unknown delivery state fails closed and releases the held terminal. A plain answer, a visible tool-call name, or HTTP 200 never proves delivery state.
5. The first response has not already consumed the one L2 attempt for this trusted epoch, the deadline and token/cost budgets leave room for it, and the canonical context can be reconstructed from existing structured host/adapter state without retaining a private full-response copy.

The delivery-call rule is intentionally temporal rather than a naive “this turn never called a delivery tool” test. The local observation records the last delivery call and the last tool execution by monotonic stream sequence:

- If there was no delivery call, a trusted owed stop tail is a normal L2 candidate.
- If the last delivery call occurred after the last tool execution (or no tool execution followed it), the result has already been delivered; release the held terminal with no second request. This covers a delivery call at the end of a normal tool turn.
- If the last delivery call occurred before a later tool execution, it is an opening acknowledgement such as the SendToUser in the incident. It does not satisfy the final result, so the later pure-text stop tail remains a candidate when deliveryOwed is true. This is the deliberate exception to the simple “any delivery call skips” rule.
- If the stream ends with a tool call or finish_reason="tool_calls", release regardless of the earlier chronology. A delivery call in an earlier event cannot justify hiding a terminal tool-call finish.

Thus the trigger is “this response ends in pure-text silence,” not “this round never called a delivery tool.” A delivery call after the final tool execution suppresses remediation; a delivery call before later tool execution does not.
...
for each event from the incremental reverse adapter:
    observe finish, delivery calls, tool executions, and sequence numbers
...
if !permission or terminalDecisionSaysSkip or mode != enforce:
...
The non-functional requirement is explicit: normal turns with a final delivery (delivery after the last tool execution) or a terminal tool-call finish have zero extra model calls, zero whole-response buffering, and zero first-byte delay. They pay only the local millisecond-scale terminal observation and release. Non-person and excluded turns do not even enter the intervention decision.

Only turns that would otherwise end in pure-text silence can incur the extra model call and its timeout. The first text/reasoning/tool deltas for those turns have already been delivered, so the added wait is confined to a turn that otherwise has no user-visible completion. A prior opening SendToUser followed by later tool execution is classified by the temporal rule above: it remains eligible, while a final delivery after the last tool suppresses L2. This is the intended blast-radius boundary.
...
TurnObservation {
  ...
  assistantToolCalls: [{ name, id, args, choiceIndex, sequence }]
  ...
  lastDeliveryCallSequence: number | "unknown"
  lastToolExecutionSequence: number | "unknown"
...
Only a trusted delivery-observed event may clear an obligation. A call-emitted delivery call, a 200 response, or a plain assistant answer does not. ReactToMessage remains in the owed-clearing set exactly as official, although it is not user-visible text. SendToUser and SendMessage are the user-visible delivery set. The L2 temporal rule separately compares call and execution sequences: a delivery after the last tool execution suppresses the silent-tail remedy, while an earlier acknowledgement before later tool execution does not. These predicates, the unknown path, and the terminal-release invariant must be unit-tested independently.
...
lastDeliveryCallSequence / lastToolExecutionSequence
...
2. Enable dry-run for one bot or a small allow-list and inspect eligible rate, family distribution, cache impact, temporal delivery classification, and possible false positives. Confirm first-byte timing is unchanged.
...
2. **Tool normalization and chronology:** canonical names and JSON/SSE/Responses/Anthropic shapes; never count a string in assistant content as a call; record monotonic lastDeliveryCallSequence and lastToolExecutionSequence; assert delivery-after-last-tool suppresses while an earlier delivery followed by later tool execution remains eligible when the tail is pure text.
...
6. **L2 decision:** person-opened permission plus terminal finish_reason=stop plus pure-text silent tail plus explicit trusted deliveryOwed; cover no delivery call, delivery after final tool, opening delivery before later tool execution, terminal tool calls, non-stop finishes, hidden/non-person, missing epoch, unknown ledger, and client state. Assert chronology rather than a “never called delivery” shortcut.
...
11. **Observability:** assert identityGateResult, distinct skipReason/classificationSkippedReason values (including no_bot_id, group_chat, terminal_tool_calls, delivery_after_last_tool, and budget_*), terminalDecision, terminalReleaseReason, heldTerminalBytes, firstResponseHash, l2BodyHash, sequence fields, and trusted deliveryObservedAt are present/unknown exactly as specified.
...
| --- | --- |
| Normal delivery/tool turn | First text/reasoning/tool deltas reach the host at their original times; only the terminal event is briefly held/released, no second request is made, no whole-response capture exists, and first-byte timing is unchanged. |
| No delivery / pure-text stop / delivery owed | First deltas are already visible; only the terminal event is held; exactly one second upstream request is issued in the same host request; the canonical context ends with [SAND_HIDDEN_PROMPT] directly followed by the exact §3.27 body. |
| Opening delivery then later tool execution, pure-text stop | The last delivery sequence is before the last tool execution, so L2 triggers. This incident-shaped case must not be rejected by a “never called delivery” predicate. |
| Delivery after the last tool execution | The response is considered delivered; the terminal event is released immediately, exactly one upstream request is made, and no L2 call occurs. |
...
| Second response has no delivery | Return the second valid response once, mark unresolved/finalNoTool, and do not start a third request or select L3 for the same epoch/tail. |
...
| Budget and permission skip | For non-person/hidden/group/subagent/routine/silence, unknown ledger, lease contention, prompt/completion/cost budget refusal, or terminal decision timeout, release the held terminal immediately, make no second request, and keep ordinary streaming behavior. |
...
3. Use deterministic fixtures for each of the three protocols and both JSON/SSE encodings to reproduce the incident response (finish_reason=stop, opening delivery before later tool execution, pure-text terminal tail) with an explicit trusted host ledger snapshot, or reproduce the same instruction on the box if the model can be controlled. The live model smoke test is not a substitute for the deterministic fixtures.
...
5. Open the control page logs and pair the custom-host/hop rows. A normal productive row must show terminalDecision=normal-release, l2AdditionalRuns=0, heldTerminalBytes returning to 0, and no first-byte delay. An incident-shaped row must show injectionFamilies=[l2.reply-nudge], terminalDecision=l2-triggered, l2AdditionalRuns=1, l2Outcome=call-emitted, firstResponseHash, l2BodyHash, and the delivery chronology sequences. If the host emits a trusted success event, the ledger may later show delivery-observed with its deliveryObservedAt; without it, owed/delivery state must remain unknown rather than an inferred transition. The host-visible transcript must contain an actual SendToUser/SendMessage tool event, not just the nudge text.
...
7. Exercise the complete three-protocol × (JSON/SSE) matrix: normal delivery/tool turns, stop/no-tool/owed, opening delivery before later tools, delivery after final tool, terminal tool-call finish, second-run success, every failure path, and over-budget fixtures. Each protocol must use the shared classifier, its existing request/response converters, and one legal host response/stream; no protocol is skipped merely because of its type.
...
- **Normal-turn gate:** 100% of ordinary turns with a final delivery or terminal tool-call finish forward the first non-terminal byte without waiting for full response completion, allocate no whole-response storage, make zero second upstream requests, and release the terminal after only local millisecond-scale work.
- **Silent-tail gate:** 100% of eligible trusted person-opened pure-text stop tails issue exactly one bounded second attempt, never more than the hard cap; no delivery call or an opening delivery before later tool execution is classified according to the temporal rule.
- **Suppression gate:** 100% of turns whose last delivery call occurs after the last tool execution make zero L2 calls and release the held terminal immediately.
...
- **Protocol/encoding matrix gate:** each of chat-completions, Responses, and Anthropic in JSON and SSE normalizes through the existing converters, applies the same finish/tool-call/chronology predicates, and either performs the bounded L2 attempt or follows the immediate terminal-release path.
...
5. **Historical chronology:** the incident's opening SendToUser must not suppress remediation after later tool execution, while a delivery after the final tool must suppress it. Use host-boundary sequence numbers and paired execution evidence; never reduce the rule to a total delivery-call count.
...
2. Add the canonical ledger/classifier with JSON/SSE/provider fixtures, including host-boundary event sequence tracking for delivery calls and tool execution.
3. Add L1 in a post-conversion policy stage, with dry-run metadata.
4. Add an incremental host-boundary reverse-adapter path for all three protocol adapters. Forward every non-terminal delta immediately; hold only finish-bearing terminal events and the required stream terminator. At the hold point, run the local person/chronology/finish decision and either release immediately or issue one same-request L2 call with the exact hidden suffix. Never add whole-response capture, a size-based branch, or first-byte delay.
