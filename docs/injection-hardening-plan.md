...
- **L2 — after generation:** the key patch for this incident. When a trusted person-opened upstream run ends with `finish_reason="stop"`, the first response contains **zero assistant `tool_calls` of any kind**, and the trusted host ledger says delivery is owed, append the exact §3.27 nudge and run the upstream once more in the same host request. The same canonical mechanism supports all three protocols (`chat-completions`, `responses`, and `anthropic`): the second request uses the existing request adapter, while the first response is normalized to a host-side chat-completions shape for shared classification and can be replayed through the existing reverse adapter if the retry fails. This plan defines no provider-continuation protocol: a first response containing even a non-delivery tool call is ineligible. Return the second response when it is valid; otherwise use a host-boundary-equivalent replay; never manufacture a tool call.
...
- Stream response direction: `src/hop/protocol-converters.ts:149`/`:150` export `responsesSseToChat(raw)` and `anthropicSseToChat(raw)`. They accept provider raw SSE text and directly produce chat-completions SSE text, including the terminal chunk and `[DONE]`.

Accordingly, L2 first buffers the provider response only after the request-level pre-screen (§5.3) admits it, passes that captured response through the existing reverse converter, and obtains both (a) a host-side chat-completions shape for shared classification and (b) host-boundary bytes for fallback. No new multi-protocol fallback path is needed. Protocol differences belong in adapter fixtures and the test matrix, not in an L2 capability gate.
...
### 5.3 L2 request-level pre-screen and buffer admission

L2 must decide whether a request can be an L2 candidate before it allocates response buffering. The pre-screen runs from the already-authenticated request context and canonical request state, before the first upstream response arrives:

- If the request is not person-opened (hidden, subagent, routine/automation, silence-allowed, or group), skip L2 and use the normal single-run path.
- If the trusted ledger already records this epoch as delivered (that is, `deliveryOwed=false`), skip L2 and use the normal single-run path.
- If botId, conversationId, epoch, ledger state, client connectivity, lease, deadline, or configuration is missing/ambiguous, fail closed and skip L2; do not turn unknown into an L2 candidate.
- Protocol type is not a pre-screen rejection: `chat-completions`, `responses`, and `anthropic` all enter the same L2 path when the request-level facts are valid.

Only a request that passes this pre-screen allocates the bounded provider-response/host-boundary capture used by enforce mode. A rejected request is streamed or returned through the existing path without an L2 buffer, so it incurs zero L2 latency. Dry-run applies the same admission decision but uses the non-blocking tee in §7.3 and keeps `bufferedBytes=0`.
...
L2 is the key patch for the incident and is a **bounded gateway-side analogue** of §3.27 across all three provider protocols, with a strict zero-tool-call first-response guard rather than a claim of full official equivalence. This plan deliberately defines no provider-continuation protocol: L2 never retries a first response that contains any `tool_calls`, including non-delivery calls. It is a second model run, not response post-processing.
...
5. The provider protocol is one of `chat-completions`, `responses`, or `anthropic`, and its existing request/response adapter participates in the shared host-boundary capture. All three protocols use the same canonical second-context construction and normalized chat-completions classifier; conversion details and fallback semantics are defined in §7.3.
...
if !l2RequestPreScreen(baseMessages, trustedContext, apiType):
    first = upstream(providerPayload(baseMessages, apiType))  # normal path; no L2 buffer
    return first unchanged
firstProvider = buffered upstream(providerPayload(baseMessages, apiType))
firstHost = existingReverseAdapter(firstProvider, apiType)
    # chat-completions is already host-shaped; Responses/Anthropic JSON/SSE
    # use responsesToChat/anthropicToChat or responsesSseToChat/anthropicSseToChat
firstSnapshot = hostBoundary(firstHost)  # host bytes, status, and headers
firstObservation = classify(normalizeChat(firstHost))
if eligible(firstObservation) and mode == enforce:
    # eligible(firstObservation) proves the normalized first response has zero tool_calls
    ...
    secondProvider = buffered upstream(providerPayload(secondMessages, apiType, bounded L2 parameters))
    secondHost = existingReverseAdapter(secondProvider, apiType)
    if secondHost is a valid response:
        return secondHost to host
    return firstSnapshot as a host-boundary-equivalent replay  # failure fallback
else:
    return firstSnapshot
~~~

The second request is built from a **complete canonical conversation**, not from `clone(body)` plus a reminder. It keeps the original model, tools, tool definitions, stream flag, and provider options unless an explicit L2 token/cost cap below lowers the output limit, and it uses the same `apiType` request adapter as the first run (raw chat-completions, `chatToResponses`, or `chatToAnthropic`). The base context already contains the prior transcript and any paired historical tool results; the selected first response is appended exactly as a plain assistant message because the zero-tool-call gate has passed. Each first response is buffered and handed to the existing reverse adapter before it is normalized to the host-side chat-completions shape for classification. If a tool call appears in any first-response choice/chunk during classification, return the first host-boundary snapshot, set `l2Eligible=false`, and record `skipReason=first_tool_call`—do not attempt to reconstruct or replay that run. Only after the complete context is assembled does the gateway append one new canonical `role: "user"` message whose content is the official hidden form **`[SAND_HIDDEN_PROMPT]` immediately concatenated with the exact §3.27 body**; no extra newline is inserted between marker and body. This is the `hidden: true → [SAND_HIDDEN_PROMPT] prepended` semantics from the read-only official catalog. The request must not remove earlier assistant text, invent a tool_calls array or tool result, set finish_reason, or change sentMessageCount locally. The second provider response is likewise handed to the existing reverse adapter; only its resulting host-boundary response is returned in the success path, so the host never sees both responses as two assistant turns.
...
### 7.3 Streaming, protocol unification, and host-boundary fallback

For a request admitted by §5.3, the L2 capture starts with the provider response (a JSON body or raw SSE text) and stays bounded by maxBufferedResponseBytes. The captured response is then handed to the existing reverse converter, producing the normalized host-side chat-completions shape/events and the host bytes used for fallback. The fallback snapshot is `{ hostBodyBytes, status, headers }` at the host boundary; do not invent a provider-specific serializer or bypass the existing converter. Provider-original bytes are never sent to the host as a fallback.

This plan enables same-request L2 for all three protocols: `chat-completions`, `responses`, and `anthropic`. For each protocol, the canonical base messages and the second context use the same request adapter as the first run; the provider response then uses the existing reverse adapter before classification and host serialization. `chat-completions` is already in the host protocol shape, while Responses and Anthropic are normalized by the converters listed in §4.4. The classifier reads only the converted chat-completions `finish_reason` and `tool_calls` presence, so all three protocols share the same predicates. Protocol type is therefore adapter mechanics, not an L2 eligibility gate; a parser, conversion, or deadline problem is handled as a classification/fallback outcome rather than a capability skip.

For JSON, buffer the provider body, apply `responsesToChat` or `anthropicToChat` when needed, and classify the resulting host-side chat-completions object; chat-completions JSON is already in that shape. For SSE, buffer the provider raw SSE text within the bound, then apply `responsesSseToChat` or `anthropicSseToChat`; these existing converters emit chat-completions SSE with the terminal chunk and `[DONE]`. Chat-completions SSE uses the same host-shape classifier directly. In enforce mode, hold the resulting host-boundary bytes until the terminal event is classified. If parsing, conversion, the client, or the first-run deadline prevents classification while a complete host snapshot is available, replay that snapshot as a host-boundary-equivalent response with its captured status/headers and log `classificationSkippedReason`; if the client is already closed, cancel and return the existing transport error.

If the bounded provider or converted host capture reaches the byte ceiling, this is not a recoverable buffered fallback: immediately switch to the normal host-boundary pass-through through the already-selected adapter, flush any converted host-visible prefix, and emit the remaining converted host-visible bytes as available. Set `l2Suppressed=buffer_overflow` and `skipReason=buffer_overflow` for this response/epoch, and **permanently skip L2 for it**. Never send a partial first stream and then append a second stream. The pass-through response uses the first host-boundary status and end-to-end headers; preserve Content-Type and Content-Encoding, and preserve Content-Length only when the unmodified host bytes and length are known. Transfer-Encoding and other hop-by-hop framing headers are proxy-owned: remove Transfer-Encoding and the Connection/Keep-Alive/TE/Upgrade family and let the server choose framing, never sending both Content-Length and Transfer-Encoding. Do not recalculate or reuse second-response headers for a first-response fallback.

If the first normalized response is eligible for any of the three protocols, send no first-response bytes to the host and run the second upstream request. On a valid second response, forward only its converted host-boundary bytes with its own valid status/headers and the original stream contract. On a second-run failure, replay the captured first snapshot as a **host-boundary-equivalent replay**: preserve semantic content, the correct terminal finish chunk, and for SSE the terminating `[DONE]`, together with the host status/content-type/framing contract. This is a semantic host-boundary replay, not a wire-level copy of the provider-original response or a hypothetical direct pass-through; protocol conversion may legitimately change bytes while preserving the host contract. First-token latency is delayed only for requests that pass the §5.3 L2 pre-screen.

Dry-run never enters the enforce full-response/provider buffer. For all three protocols it tees the host-boundary stream to the host immediately and performs only incremental, best-effort classification over normalized events (bounded parser state/counters, at most 64 KiB; `bufferedBytes=0`). It therefore does not block, mutate the request, issue a second call, or change first-token/stream latency; its overhead is O(1) CPU per chunk and at most 64 KiB of classification state. If an adapter cannot expose a non-blocking normalized event stream, dry-run records `classificationSkippedReason=non_blocking_adapter` and performs no buffering; this skips observation for that request, not the enforce capability of the protocol. The first-response hash, when available, is computed from the host-boundary bytes rather than provider-original bytes.
...
- Before dispatch, estimate the complete second prompt (including the first assistant output, tool results, and nudge). If it exceeds maxAdditionalPromptTokens, abandon L2 and return the first response as a host-boundary-equivalent replay. Set the provider's max_tokens/max_completion_tokens to no more than maxAdditionalCompletionTokens; if the adapter cannot enforce an output cap, skip L2 rather than issue an unbounded run.
...
- A non-2xx response, timeout, network error, parse/conversion error, or exhausted deadline/quota is a remediation failure: return a host-boundary-equivalent replay of the first response with its captured status/headers and semantic content and mark the epoch unresolved. Buffer overflow follows the immediate host-boundary pass-through rule above, suppresses L2 permanently for this response/epoch, and is not retried.
...
- The first response is never discarded on an L2 failure; for every protocol it is converted through the existing reverse adapter and replayed from the captured host-boundary snapshot with its host-visible status/header policy (or immediately passed through the converted host stream on buffer overflow). This is a **host-boundary-equivalent replay**—semantic content, terminal finish chunk, and SSE `[DONE]` must be correct, but wire-level identity with provider-original or direct-pass-through bytes is not a requirement. The §5.3 pre-screen rejects non-candidates before any L2 buffer is allocated.
...
bufferedBytes                 # bounded provider/host-boundary bytes held; 0 for dry-run tee/pass-through
...
| l2.maxBufferedResponseBytes | 4194304 | 262144..8388608; bounded provider-response and converted host-boundary capture for all three protocols; on overflow immediately pass through the selected adapter's converted host bytes and permanently suppress L2 for that response/epoch. |
...
5. **L2 decision:** only `finish_reason=stop` plus **zero first-response `tool_calls` of any kind** plus explicit trusted deliveryOwed plus one of the three supported protocol adapters is eligible. A stop response containing non-delivery `tool_calls` must assert `l2Eligible=false`, `skipReason=first_tool_call`, and no L2 dispatch; exercise this predicate for chat-completions, Responses, and Anthropic in both JSON and SSE. Also cover non-stop, delivery calls, hidden/non-person, missing epoch, unknown ledger, conversion/classification failure, and client bytes. No provider-continuation protocol is assumed.
...
7. **L2 bounds and host fallback:** zero/one/two configured additional runs, hard cap, timeout, prompt/output token caps, cost reservation, client close, host-boundary buffer overflow pass-through, host-boundary-equivalent first-response replay with semantic content/status/headers, valid second no-tool final, finalNoTool suppression, and no third/L3 run.
...
10. **Dry-run and protocol behavior:** JSON and streamed dry-run cases for all three protocols tee immediately with no full response/SSE buffer, no request mutation, and no first-token latency change; an adapter without a non-blocking classifier records `classificationSkippedReason` for that observation only. This is an adapter execution-path test, not a protocol capability gate.
...
| --- | --- |
| No tools / delivery owed | Normalize to chat-completions shape, require terminal stop and zero `tool_calls`, issue exactly one bounded L2 attempt, and verify the second canonical context and adapter conversion. |
| Any tool call | Shared classifier rejects L2 with `skipReason=first_tool_call`; send no second request and preserve the ordinary host response. |
| Retry failure | When the second run fails or times out, return a host-boundary-equivalent replay with semantic content, terminal finish, SSE `[DONE]`, and the host status/header contract; do not assert wire-level identity with provider-original bytes. |
| Over budget | Preflight prevents the second request, records the matching budget reason, and returns the same host-boundary-equivalent first response; no fabricated tool call or third request. |

- **No-tool success (all protocols/encodings):** for every protocol and both JSON/SSE encodings, make the first response terminal with `finish_reason="stop"`, plain assistant text, and zero `tool_calls`, with a trusted host ledger snapshot `deliveryOwed=true`. Assert exactly two upstream requests in one host request; the second canonical context contains the first assistant text and ends with a user message whose content is [SAND_HIDDEN_PROMPT] directly followed by the exact §3.27 body; the same protocol adapter converts the second request, and the host receives its response when it contains a real delivery tool call, recorded initially as call-emitted and upgraded to delivery-observed only by a host success event.
- **A stop plus non-delivery tool call is not eligible (all protocols/encodings):** each protocol's JSON and SSE fixture has `finish_reason="stop"` and one non-delivery `tool_calls` entry (even if a fixture supplies a purported result). Assert exactly one upstream request, `l2Eligible=false`, `skipReason=first_tool_call`, no §3.27 second request, and no provider-continuation inference or fabricated result.
- **Second response still has no tools (all protocols/encodings):** assert exactly two total upstream requests (initial + one default L2 attempt), `finalNoTool=true` suppresses L3 for that epoch/tail, no infinite loop, and the chosen converted final response is returned without a fabricated tool call.
- **Host-boundary fallback (all protocols/encodings):** make the second run fail or time out after the first provider response has been converted/serialized for the host. Assert a **host-boundary-equivalent replay** with semantic content, terminal finish, SSE `[DONE]`, and the captured host-visible status/end-to-end headers—not provider-original bytes; do not assert wire-level identity. Content-Length is preserved only when valid, Transfer-Encoding is proxy-normalized, the request is marked `l2_fallback`, `firstResponseHash` hashes host bytes, and no gateway-created delivery call appears.
- **SSE conversion and retry (all protocols):** for each protocol, feed provider raw SSE to the existing adapter, assert the normalized host-visible stream has terminal stop and zero tools, then return a second SSE with a tool call. Assert no first host bytes reached the host, only the second converted host-boundary stream is returned with valid content type/status/finish mapping and terminal `[DONE]`, and the call is not labeled delivery-observed without a host event.
- **Host-boundary SSE overflow (all protocols):** make each protocol's converted host-visible first SSE exceed maxBufferedResponseBytes. Assert the gateway immediately switches to adapter pass-through, emits a semantically valid converted host stream with first-response status/header semantics and terminal `[DONE]`, records `bufferedBytes` and `skipReason=buffer_overflow`, permanently suppresses L2 for that response/epoch, and never emits a second upstream request.
- **Existing conversion contract:** for Responses and Anthropic JSON, assert the provider body goes through `responsesToChat`/`anthropicToChat`; for SSE, assert raw provider text goes through `responsesSseToChat`/`anthropicSseToChat` and includes the terminal chunk plus `[DONE]`. For chat-completions, assert the existing host shape is consumed directly. All three outputs feed the same finish/tool-call predicates and the same host-boundary fallback path.
- **Dry-run JSON/SSE (all protocols):** assert the first host-visible bytes reach the host immediately, `bufferedBytes=0`, no second request or request mutation occurs, and first-token latency is unchanged; when a provider adapter cannot expose non-blocking normalized events, assert `classificationSkippedReason=non_blocking_adapter` for observation only, without declaring the protocol unsupported.
...
- **Quota fixture (all protocols/encodings):** make the complete second prompt exceed maxAdditionalPromptTokens, the bounded output exceed maxAdditionalCompletionTokens, or the worst-case cost exceed maxAdditionalCostUsd. Assert L2 is abandoned before dispatch, `skipReason` identifies `budget_prompt`, `budget_completion`, or `budget_cost`, and the first response is returned as a host-boundary-equivalent replay with no fabricated tool or third request.
...
3. Use deterministic fixtures for each of the three protocols and both JSON/SSE encodings to reproduce the incident response (finish_reason=stop, zero tool_calls) with an explicit trusted host ledger snapshot, or reproduce the same instruction on the box if the model can be controlled. The live model smoke test is not a substitute for the deterministic fixtures.
4. Switch only this bot to mode: enforce with L2 enabled and issue one person-opened turn for each protocol/encoding pair that otherwise ends in plain text. Observe one same-request L2 attempt and a second response containing a real delivery tool call; record this as call-emitted unless the host success event contract separately confirms delivery-observed.
...
7. Exercise the complete three-protocol × (JSON/SSE) matrix: stop/no-tool/owed, first-response tool call, second-run failure, and over-budget fixtures. Each protocol must use the shared classifier, its existing request/response converters, and host-boundary-equivalent fallback semantics; no protocol is skipped merely because of its type.
...
- Deterministic integration fixture: 100% of eligible first-stop/**zero-tool-call** cases across all three protocol adapters and both JSON/SSE encodings produce exactly one bounded remediation attempt; no case produces more than the configured hard cap. A first stop with any non-delivery `tool_calls` produces zero L2 attempts.
...
- Protocol/encoding matrix gate: each of chat-completions, Responses, and Anthropic in JSON and SSE normalizes through the existing converters, applies the same finish/tool-call predicates, and either performs the bounded L2 attempt or follows the defined host-boundary-equivalent fallback; a request rejected by the §5.3 pre-screen allocates no L2 buffer.
...
2. **Streaming latency/memory:** enforce-mode L2 buffering delays first-token delivery only for requests that pass the §5.3 pre-screen, across all three protocols; rejected non-person/already-delivered requests allocate no buffer and incur zero L2 latency. Host-boundary overflow immediately passes through converted bytes and permanently suppresses L2 for that response/epoch. Dry-run uses a non-blocking tee with zero full-response buffering and no first-token delay; if an adapter cannot provide that path it skips observation only. Confirm the memory ceiling, header/framing behavior, and acceptable limits for the largest provider response.
3. **Provider shape drift:** finish reasons and tool-call deltas differ across chat-completions, Responses, and Anthropic. Existing bidirectional converters normalize those differences before one shared classifier; retain JSON/SSE fixtures for every adapter and treat this as a test-coverage/adapter correctness risk, not an L2 capability gap. Conversion or classification errors use the bounded host-boundary fallback and are logged explicitly.
...
4. Add L2 buffered classification and same-request bounded retry for all three protocol adapters when the normalized first response has zero tool_calls, including complete context, direct hidden-marker concatenation, token/cost preflight, host-boundary-equivalent first-response fallback (semantic content/status/headers), and converted-byte overflow pass-through; use the existing request/reverse converters and keep any first tool call ineligible.
