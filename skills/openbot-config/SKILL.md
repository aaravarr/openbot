...
Hop request behavior: host image parts (user attachments) become standard OpenAI `image_url` content, and a Read-style tool call targeting an image file (`.png`/`.jpg`/`.jpeg`/`.webp`/`.gif`) whose tool result carries no image data gets a `data:image/…` user message injected after that tool result (20 MB cap; missing/oversize files are skipped). Injected images are auto-compressed (JPEG q85, long edge ≤1568) and the request is held under a 9 MiB byte budget so the upstream 10 MiB limit never 413s. Details and guards: [reference.md](reference.md) § "Image handling on the custom hop path".
